...
function mysqli_query_log($link,$sql) { 
	//echo $sql;
	$link_p = mysqli_connect('localhost', 'prape', '452562ab', 'prape'); 
	//쿼리 호출하는 페이지명 함께 삽입 
...
function save_img($table_name, $column, $key){
	...
	if(!empty($_FILES['pet_image'])){ 
		...
		//1. 기존의 해당 컬럼에 이미지가 있으면 삭제 후, 업로드 및 업데이트 해야함 
		//delete_img($table_name, $column, $key);
		...
		// 2. 파일저장 
			...
			if (move_uploaded_file($_FILES['pet_image']['tmp_name'], $uploadfile)) { 
				...
				//파일 업로드 후, 수정일 경우 기존파일 삭제 (0906수정)
				$s_sql="SELECT $column FROM `$table_name` WHERE num ='$key'";
				$s_result = mysqli_fetch_row(mysqli_query($link,$s_sql));
				$s_img =$s_result[0];
				...
				if($s_img){ 
					...
					$sql="SELECT count(*) FROM `$table_name` WHERE $column = '$s_img'";
					//$sql="SELECT COUNT($column)AS img_cnt FROM `$table_name` WHERE $column = (SELECT $column FROM `$table_name` WHERE num ='$key') GROUP BY img_cnt";
